    ...
    def forward(self, input_ids: torch.LongTensor, s_mask: torch.BoolTensor,
        ...
        if module == 'module3': 
            if mse_loss:
                ...
                #print('mse_pred shape: ', mse_pred.shape)
                #print('mse_target shape: ', mse_target.shape)
                #print('mse_pred: ', mse_pred[0,0,:3])
                #print('mse_target: ', mse_target[0,0,:3])
                mse_loss_val = F.mse_loss(mse_pred, mse_target)
                #print('mse loss: ', mse_loss_val)
                output.ce_loss = torch.tensor(0.0, device=hidden_states.device)
                output.mse_loss = mse_loss_val
                output.loss = mse_loss_val  
        if module == 'module2':
...
class TwoStageModel(nn.Module):
    def __init__(self, module1: STokenGPTModel, module2: STokenGPTModel, module3: STokenGPTModel,
    ...
    def forward(self, input_ids: torch.LongTensor, s_mask: torch.BoolTensor, labels: torch.LongTensor = None):
            ...
            #print('out2 loss: ', out2.loss)
            #print('out recurrent loss: ', out_recurrent.loss)
            regular = 0.1
            loss = (1-regular)*out2.loss + regular*out_recurrent.loss
            return SimpleNamespace(loss=loss, ce_loss=out2.ce_loss, mse_loss=out_recurrent.mse_loss, logits=out2.logits)
...
def main():
    ...
    module3 = STokenGPTModel(config1)
    # Build composite two-stage model.
    composite_model = TwoStageModel(module1, module2, module3, window=args.window,
                                    text_run=args.text_run, state_run=args.state_run)
