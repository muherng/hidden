...
def test():
    # build a simple sequence [1,2,3,4,5]
    L = 8
    seq = torch.randint(0, 10, (1, L, 1, 1), dtype=torch.float32)
    
    # A=1 ⇒ H[t] = H[t-1] + X[t]
    A = torch.ones(1, L, 1, 1, dtype=torch.float32)
    X = torch.tensor(seq, dtype=torch.float32).view(1, L, 1, 1).requires_grad_(True)
    
    # forward
    out = pscan(A, X)  # shape (1, L, 1, 1)
    expected = torch.cumsum(X, dim=1)
    assert torch.allclose(out, expected), f"Forward failed: {out.view(-1)} != {expected.view(-1)}"

    print('Input:', X.view(-1))
    print('Output:', out.view(-1))
    print('Expected:', expected.view(-1))
    
    # backward: sum(out) ⇒ dL/dX[k] = number of outputs ≥ k+1 = L-k
    out.sum().backward()
    expected_grad = torch.tensor([L - i for i in range(L)], dtype=torch.float32).view(1, L, 1, 1)
    assert torch.allclose(X.grad, expected_grad), (
        f"Backward failed: {X.grad.view(-1)} != {expected_grad.view(-1)}"
    )
