...
class TextWithSTokenDataset(Dataset):
    def __init__(self, split, tokenizer, seq_len, k):
        ...
        for i in range(0, len(self.token_ids) - seq_len, seq_len):
            ...
            input_ids_list = []  # text tokens: token id; s tokens: -1.
            s_mask_list = []     # True if position is an s token.
            labels_list = []     # text tokens: token id; s tokens: -100.
            for j, token in enumerate(a_tokens):
                ...
                if (j + 1) % self.k == 0 and (j + 1) < len(a_tokens):
                    ...
                    labels_list.append(-100)   # mask loss for s tokens.
            sample = {
...
#    (Used as both module1 and module2)
# -----------------------------------------------------------------------------
...
class NoExtraLayerSTokenGPTModel(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # For s tokens, we use a fixed continuous vector (initialized as zeros).
    ...
    def forward(self, input_ids: torch.LongTensor, s_mask: torch.BoolTensor,
                ...
                override_s: torch.Tensor = None):
        """
        If override_s is provided, then for positions where s_mask is True, use override_s.
        If return_hidden is True, return the final hidden states along with logits.
        """
...
class TwoStageModel(nn.Module):
    def __init__(self, module1: NoExtraLayerSTokenGPTModel, module2: NoExtraLayerSTokenGPTModel):
        ...
        module2 takes p'' as input where s token positions are replaced with s' from module1.
        """
        super().__init__()
    ...
    def forward(self, input_ids: torch.LongTensor, s_mask: torch.BoolTensor, labels: torch.LongTensor = None):
        # Module1 pass: obtain hidden states.
        out1 = self.module1(input_ids=input_ids, s_mask=s_mask, labels=labels, return_hidden=True)
        # Module2 pass: use module1's hidden states to override s token positions.
...
# -----------------------------------------------------------------------------
# 5. PrintLossCallback for Logging
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 6. Custom Trainer Subclass (Override compute_loss)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 7. Main Training Script with Timestamp Logic
# -----------------------------------------------------------------------------

def main():
    parser = argparse.ArgumentParser(
        description='Train a two-stage transformer model: module1 and module2 with s tokens replaced by module1 outputs.'
    )
